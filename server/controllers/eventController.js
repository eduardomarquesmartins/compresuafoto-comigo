            ...
            (async () => {
                ...
                await Promise.all(photos.map(file => limit(async () => {
                    try {
                        ...
                        const originalUpload = await cloudinaryService.uploadStream(buffer, `events/event_${event.id}/originals`);

                        // 2. Generate Watermark
                        const watermarkedBuffer = await watermarkService.generateWatermark(buffer);
                        const watermarkUpload = await cloudinaryService.uploadStream(watermarkedBuffer, `events/event_${event.id}/watermarked`);
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                // Upload original para Cloudinary (pasta específica do evento)
                const cloudResult = await cloudinaryService.uploadStream(buffer, `events/event_${event.id}`);
                ...
                try {
                    ...
                    // Se > 5MB, redimensionamos para um tamanho seguro
                    let bufferForRekognition = buffer;
                    if (buffer.length > 5 * 1024 * 1024) {
                        logToFile('File > 5MB, resizing for Rekognition...');
                        bufferForRekognition = await sharp(buffer).rotate().resize(1200).toBuffer();
                    } else {
                        // Mesmo se for pequena, forçamos a rotação EXIF para alinhar o rosto
                        bufferForRekognition = await sharp(buffer).rotate().toBuffer();
                    }
                ...
                if (results.success === 0 && !event.coverImage) {
                    await prisma.event.update({
                        where: { id: event.id },
                        data: { coverImage: cloudResult.secure_url }
                    });
                }
