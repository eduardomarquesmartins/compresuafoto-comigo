...
exports.createOrder = async (req, res) => {
    try {
        ...
        if (serverTotal === 0) {
            // Send email for free order if user is logged in
            if (userId) {
                const user = await prisma.user.findUnique({ where: { id: userId } });
                if (user && user.email) {
                    console.log(`[DEBUG] Disparando e-mail para pedido gratuito: ${user.email}`);
                    await emailService.sendOrderEmail(user.email, result.publicId || result.id, photos.length);
                }
            }
