...
export default function EventDetailPage() {
    ...
    const getPhotoUrl = (photo: any) => photo.url;

    // Simple toggle for this page (doesn't use global cart to keep it simple as original)
    const toggleSelection = (photo: any) => {
        console.log('Toggled', photo.id);
        // Original didn't have a selection state, just a buy button
    };
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                {visibleResults.map((photo) => (
                                    <PhotoGridItem
                                        key={photo.id}
                                        photo={photo}
                                        isSelected={false} // Simple version doesn't track selection
                            ...
                            {visibleCount < results.length && (
                                <div className="flex justify-center mt-12">
                                    <button
                                        onClick={loadMore}
                                        className="bg-slate-800 border border-slate-700 hover:border-brand text-white px-8 py-3 rounded-full font-medium text-sm transition-all"
                                    >
                                        Carregar Mais Fotos
                                    </button>
                                </div>
