...
function OrderSuccessContent() {
    ...
    const orderId = searchParams.get("external_reference"); // From Mercado Pago
        ...
        return (
            <div className="min-h-screen bg-black text-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-400 mt-2">Verifique se o link está correto.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-black text-white font-sans p-6 flex flex-col items-center pt-20">
            <div className={`max-w-3xl w-full bg-slate-900 border ${isPaid ? 'border-green-500/50' : 'border-yellow-500/50'} rounded-3xl p-8 mb-8 text-center`}>
                ...
                {isPaid ? (
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-300 mb-6">Suas fotos foram desbloqueadas. Você pode baixá-las abaixo.</p>
                        ...
                        <div className="bg-black/40 rounded-xl p-4 flex items-center gap-4 border border-white/5">
                            ...
                            <div className="text-left">
                                ...
                                <p className="font-bold text-green-400">R$ {order.total.toFixed(2)}</p>
                            </div>
                        </div>
            ...
            {isPaid && (
                <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {order.photos.map((photo) => (
                        <div key={photo.id} className="bg-slate-900 border border-white/10 rounded-2xl overflow-hidden group">
                            ...
                            <div className="p-4 flex items-center justify-between">
                                <span className="text-sm text-gray-400">Foto #{photo.id}</span>
                                <span className="text-xs bg-green-500/20 text-green-400 px-2 py-1 rounded-full uppercase tracking-wider font-bold">Desbloqueada</span>
                            </div>
                        </div>
                    ))}
                </div>
...
export default function OrderSuccessPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="w-10 h-10 text-brand animate-spin" /></div>}>
            <OrderSuccessContent />
        </Suspense>
    );
}
