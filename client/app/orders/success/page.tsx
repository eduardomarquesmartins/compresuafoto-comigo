...
function OrderSuccessContent() {
    ...
    useEffect(() => {
        if (orderId) {
            // If we have an approved status in URL AND a paymentId, verify it server-side
    ...
    const syncPayment = async (id: string, payId: string) => {
        try {
            // Call the new sync endpoint that verifies status with Mercado Pago server-to-server
            const res = await api.post(`/orders/${id}/sync?payment_id=${payId}`);
            setOrder(res.data);
        } catch (error) {
            console.error("Erro ao sincronizar pagamento:", error);
            // Fallback to basic fetch if sync fails
            fetchOrder(id);
        } finally {
            setLoading(false);
        }
