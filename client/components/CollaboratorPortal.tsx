...
import { Check, ClipboardList, Download, LogOut, Plus, Trash2, Wallet } from "lucide-react";
...
export default function CollaboratorPortal({ role, title }: { role: "DESIGNER" | "DEMANDAS"; title: string }) {
    ...
    const [receiptCompetence, setReceiptCompetence] = useState(new Date().toISOString().slice(0, 7));
    ...
    const load = async () => {
        try {
            ...
            if (data.completions[0]?.completedAt) setReceiptCompetence(new Date(data.completions[0].completedAt).toISOString().slice(0, 7));
        } catch { setMessage("Não foi possível carregar seus serviços."); }
    };
    useEffect(() => { load(); }, []);
    useEffect(() => {
        const input = document.querySelector<HTMLInputElement>('input[aria-label="Competência do recibo"]');
    ...
    const downloadReceipt = async () => {
        setDownloadingReceipt(true); setMessage("");
        try {
            const response = await api.get("/collaborators/portal/receipt", { params: { competence: receiptCompetence }, responseType: "blob" });
            const url = URL.createObjectURL(new Blob([response.data], { type: "application/pdf" }));
            const link = document.createElement("a");
            link.href = url; link.download = `recibo-servicos-${receiptCompetence}.pdf`; link.click();
            URL.revokeObjectURL(url);
        } catch (error: any) {
            const responseData = error.response?.data;
            const detail = responseData instanceof Blob ? await responseData.text().then(text => {
                try { return JSON.parse(text).error; } catch { return ""; }
            }) : responseData?.error;
            setMessage(detail || "Não foi possível gerar o recibo para esta competência.");
        }
